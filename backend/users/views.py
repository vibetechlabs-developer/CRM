...
class TicketStatsAPIView(APIView):
    """
    Explicit endpoint used by frontend `UserControl` popup.
    This duplicates the logic from `UserViewSet.ticket_stats` to avoid router/action
    generation issues in some deployments.
    """
    ...
    def get(self, request):
        ...
        completed_by_user_agg = (
            TicketActivity.objects.filter(user_id__in=user_ids)
            .filter(Q(message__icontains="Status changed") & Q(message__icontains=ticket_status_completed))
            .values("user_id")
            .annotate(completedByUserTicketsCount=Count("ticket_id", distinct=True))
        )
        completed_by_user_map = {row["user_id"]: row["completedByUserTicketsCount"] for row in completed_by_user_agg}
